...
# torch.manual_seed(1)
from transformers import BertModel, AlbertModel, BertForTokenClassification
...
class BERT_CRF(nn.Module):
    ...
    def __init__(self, args):
        ...
        if self.bert_model_name.startswith('bert-'):
            ...
            self.word_embeds = AlbertModel.from_pretrained(self.bert_model_name)
            print('load pre-trained model of {}'.format(self.bert_model_name))
        else:
    ...
    def _get_lstm_features(self, sentence, lengths):  # (batch_size, seq_length)
        ...
        lstm_out, hidden = self.lstm(embeds)  # (seq_length, batch_size, hidden_size)
        # lstm_out_padded, _ = pad_packed_sequence(lstm_out)
        lstm_feats = self.hidden2tag(lstm_out)  # (seq_length, batch_size, tag_size)
